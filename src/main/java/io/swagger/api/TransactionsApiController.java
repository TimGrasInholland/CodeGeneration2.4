...
public class TransactionsApiController implements TransactionsApi {
    ...
    public ResponseEntity<List<Transaction>> getAllTransactions(@ApiParam(value = "transactions to date") @Valid @RequestParam(value = "dateTo", required = false) LocalDate dateTo
        ...
        if (accept != null && accept.contains("application/json")) {
            try {
                if (request.getQueryString().contains("limit") && request.getQueryString().contains("offset")) {
                    ...
                    return ResponseEntity.status(200).body(service.getAllTransactionsBetweenDates(dateFrom, dateTo));
                }
            } catch(Exception e) {
